...
export async function getSessionCards(sessionId: string): Promise<SessionCardsResult> {
  ...
  return withUser(async ({ user, supabase }) => {
    try {
      ...
      return { ok: false, error: { message: err instanceof Error ? err.message : 'Terjadi kesalahan', code: 'UNKNOWN_ERROR' } }
    }
  }, RATE_LIMITS.standard) as Promise<SessionCardsResult>
}
...
export async function publishCards(cardIds: string[]): Promise<PublishCardsResult> {
  ...
  }, RATE_LIMITS.sensitive) as Promise<PublishCardsResult>
}
...
export async function archiveCards(cardIds: string[]): Promise<ArchiveCardsResult> {
  ...
  }, RATE_LIMITS.sensitive) as Promise<ArchiveCardsResult>
}
...
export async function duplicateCard(cardId: string): Promise<DuplicateCardResult> {
  ...
  }, RATE_LIMITS.sensitive) as Promise<DuplicateCardResult>
}
...
export async function getSessionStats(sessionId: string): Promise<{
  ...
  }, RATE_LIMITS.standard) as Promise<{
    ok: boolean
    stats?: { draftCount: number; publishedCount: number; archivedCount: number; totalCount: number; questionNumbers: number[] }
    error?: { message: string; code: string }
  }>
}
