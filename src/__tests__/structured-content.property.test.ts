...
// ============================================
// Matching Set Detection Properties
// ============================================

import {
  detectMatchingBlocks,
  isMatchingSetBlock,
  formatMatchingOptionsForPrompt,
} from '@/lib/matching-set-detector'

describe('V11 Structured Content - Matching Set Detection Properties', () => {
  /**
   * **Feature: v11-Structured-Content-Engine, Property 13: Matching Block Detection**
   * *For any* text containing a contiguous block of labeled options (A., B., C.) 
   * followed by numbered questions, the detectMatchingBlocks function SHALL 
   * identify and return the block.
   * **Validates: Requirements 8.1**
   */
  describe('Property 13: Matching Block Detection', () => {
    it('detects basic matching block with A-E options', () => {
      const text = `
A. First option
B. Second option
C. Third option
...
1. Question referencing options
2. Another question
3. Third question
      `.trim()
      ...
      if (blocks.length > 0) {
        expect(blocks[0].optionLabels).toContain('A')
        expect(blocks[0].optionLabels).toContain('B')
        expect(blocks[0].questionNumbers.length).toBeGreaterThan(0)
      }
    })

    it('detects options with various label counts', () => {
      fc.assert(
        fc.property(
          fc.integer({ min: 2, max: 6 }), // Number of options (A-F)
          fc.integer({ min: 1, max: 5 }), // Number of questions
          (optionCount, questionCount) => {
            const labels = 'ABCDEFGHIJ'.slice(0, optionCount).split('')
            const optionLines = labels.map((l, i) => `${l}. Option ${i + 1} text`).join('\n')
            const questionLines = Array.from({ length: questionCount }, (_, i) => 
            ...
            if (blocks.length > 0) {
              expect(blocks[0].optionLabels.length).toBe(optionCount)
              expect(blocks[0].questionNumbers.length).toBe(questionCount)
            }
          }
        ),
        { numRuns: 50 }
    ...
    it('returns empty for text without matching pattern', () => {
      fc.assert(
        fc.property(
          fc.string().filter(s => 
            !/^[A-Z][.)]\s/m.test(s) && // No option labels
            s.length < 500
          ),
          (text) => {
            const blocks = detectMatchingBlocks(text)
    ...
    it('isMatchingSetBlock returns true for valid matching blocks', () => {
      const validBlock = `
A. Option A
B. Option B
C. Option C
...
2. Second question
      ...
      expect(isMatchingSetBlock(validBlock)).toBe(true)
    })

    it('isMatchingSetBlock returns false for non-matching text', () => {
      const nonMatching = 'Just some regular text without any matching patterns.'
      expect(isMatchingSetBlock(nonMatching)).toBe(false)
    })
  })

  /**
   * **Feature: v11-Structured-Content-Engine, Property 14: Matching Block Card Options**
   * *For any* card generated from a matching block, the card's options array 
   * SHALL contain all options from the matching block's common_options.
   * **Validates: Requirements 8.3**
   */
  describe('Property 14: Matching Block Card Options', () => {
    it('formatMatchingOptionsForPrompt includes all options', () => {
      fc.assert(
        fc.property(
          fc.array(fc.string().filter(s => s.trim().length > 0), { minLength: 2, maxLength: 6 }),
          (optionTexts) => {
            const labels = 'ABCDEFGHIJ'.slice(0, optionTexts.length).split('')
            const block = {
              optionLabels: labels,
              optionTexts,
            ...
            const formatted = formatMatchingOptionsForPrompt(block)
            
            // All options should be in the formatted string
            for (let i = 0; i < optionTexts.length; i++) {
              expect(formatted).toContain(labels[i])
              expect(formatted).toContain(optionTexts[i])
            }
          }
        ),
        { numRuns: 50 }
